...
Two built-in boards; see `GET /api/v1/board-models`.
...
| `INVALID_JUMPER_ENDPOINT`     | error    | A jumper references a missing or disabled hole. |
| `CRITICAL_NET_CLASS_ROUTED`   | warning  | A `high-current`/`clock`/`switching`/`analog-sensitive` net was routed. |
| `HIGH_CURRENT_ON_RAIL`        | warning  | A `high-current` net reaches a rail. |
| `HIGH_CONGESTION`             | warning  | A column bucket carries more than 6 jumpers. |

See `backend/app/domain/diagnostics.py` for the full catalog.